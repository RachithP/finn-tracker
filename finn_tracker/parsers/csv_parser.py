...
def _extract_last4_from_name(name: str) -> str:
    ...
    Skips year-like groups (1900-2099) so a trailing year doesn't shadow the account number."""
    groups = re.findall(r'(?<!\d)(\d{4})(?!\d)', name)
    non_year = [g for g in groups if not re.match(r'^(19|20)\d{2}$', g)]
    return non_year[0] if non_year else (groups[0] if groups else "")
...
def _find_header_skip(text: str) -> int:
    """Scan up to _MAX_HEADER_SCAN non-empty lines to find the CSV header row.

    Returns the line index (for pd.read_csv's skiprows) of the first line whose
    comma-separated tokens contain a known format's required columns or any
    standard date synonym.

    Raises ValueError if no plausible header row is found within the limit — the
    file either has too many preamble rows or no recognisable header at all.
    """
    scanned = 0
    for i, line in enumerate(text.splitlines()):
        ...
        # Try each known format first (most specific check)
        for fmt_name, cfg in FORMATS.items():
            if fmt_name == "generic":
                continue
            if cfg["required_cols"] and {c.lower() for c in cfg["required_cols"]}.issubset(cols):
                return i
        # Fall back to generic: any recognised date synonym is enough
        if any(s in cols for s in DATE_SYNONYMS):
            return i
        if scanned >= _MAX_HEADER_SCAN:
            raise ValueError(
                f"No column headers found in first {_MAX_HEADER_SCAN} non-empty rows"
            )
    return 0  # very short or empty file — let pd.read_csv handle it
...
def parse_csv(file_path: str, account_label: str = "") -> ParseResult:
    ...
    try:
        ...
        skip = _find_header_skip(text)  # raises ValueError after _MAX_HEADER_SCAN rows
        df = pd.read_csv(io.StringIO(text), dtype=str, skiprows=skip)
