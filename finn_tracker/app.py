...
def _scan_default_folders() -> List[dict]:
    """Scan auto-load folders using db.py's mtime-cached scan_folders.
    Enrichment (category overrides, learned rules, autocat) is applied fresh
    on every call so category edits take effect without invalidating the cache."""
    return [_enrich(t) for t in scan_folders()]
...
def get_transactions():
    ...
    combined = dedup(folder_txns + _session["user_transactions"])
...
def check_duplicates():
    ...
    existing_keys = {
        (t.get("date"), t.get("merchant", "").lower().strip(), round(float(t.get("amount", 0)), 2))
        for t in existing
    }

    duplicates = []
    for new_t in new_batch:
        key = (new_t.get("date"), new_t.get("merchant", "").lower().strip(), round(float(new_t.get("amount", 0)), 2))
...
def batch_update_categories():
    ...
    all_cats = set(DEFAULT_CATEGORIES) | set(_session["custom_categories"])
    valid = [
        (u.get("txn_id", "").strip(), u.get("category", "").strip())
        for u in updates
        if u.get("txn_id", "").strip() and u.get("category", "").strip() in all_cats
    ]
    if valid:
        ts = datetime.now().isoformat()
        with _db_conn() as conn:
            conn.executemany(
                "INSERT INTO category_overrides (txn_id, category, updated_at) VALUES (?, ?, ?)"
                " ON CONFLICT(txn_id) DO UPDATE SET category=excluded.category, updated_at=excluded.updated_at",
                [(tid, cat, ts) for tid, cat in valid],
            )
        for tid, cat in valid:
            _session["categories"][tid] = cat
    return jsonify({"updated": len(valid), "ok": True})
...
LLAMA_CPP_TIMEOUT = int(os.environ.get("LLAMA_CPP_TIMEOUT", "120"))
...
def init_app() -> None:
    """Initialize the DB and load session state. Called by __main__.py at startup
    and by test fixtures (after redirecting DB_PATH) to avoid module-level side effects."""
    _init_db()
