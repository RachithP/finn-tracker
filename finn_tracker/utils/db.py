...
# Per-file mtime cache: Path → (mtime_float, raw_dicts_without_txn_id_or_overrides)
_scan_cache: Dict[Path, Tuple[float, List[dict]]] = {}


def scan_folders(folders: List[Path] = None) -> List[dict]:
    """
    Scan CSV/PDF files from the default (or given) folders.

    Returns raw parsed dicts with account and source_folder set, but WITHOUT
    txn_id or category overrides applied — callers handle those. Results are
    cached per file by mtime; the cache is invalidated when a file changes.
    ...
    for folder in folders:
        ...
        for fp in sorted(p for p in folder.iterdir() if p.suffix.lower() in {".csv", ".pdf"}):
            ...
            if fp in _scan_cache and _scan_cache[fp][0] == mtime:
                raw = _scan_cache[fp][1]
            else:
                ...
                for t in result.transactions:
                    ...
                    raw.append(d)
                _scan_cache[fp] = (mtime, raw)

            # Shallow copy so callers don't mutate cached entries
            txns.extend(dict(d) for d in raw)
...
def dedup(txns: List[dict]) -> List[dict]:
    """Deduplicate by (date, merchant lower, signed amount, account). Preserves first occurrence."""
    seen, out = set(), []
    for t in txns:
        key = (
            t.get("date"),
...
def get_all_transactions(db_path: Path = None) -> List[dict]:
    ...
    folder_txns = scan_folders()
    # Apply txn_id and overrides to folder transactions (not cached — overrides change at runtime)
    for t in folder_txns:
        tid = make_txn_id(t)
        t["txn_id"] = tid
        if tid in overrides:
            t["category"] = overrides[tid]
