...
class TestDedup(unittest.TestCase):
    ...
    def test_different_accounts_not_deduped(self):
        t1 = {"date": "2024-01-01", "merchant": "STORE", "amount": -50.0, "account": "Chase ••1234"}
        t2 = {"date": "2024-01-01", "merchant": "STORE", "amount": -50.0, "account": "BofA ••5678"}
        self.assertEqual(len(dedup([t1, t2])), 2)

    def test_same_account_duplicate_removed(self):
        t1 = {"date": "2024-01-01", "merchant": "STORE", "amount": -50.0, "account": "Chase ••1234"}
        t2 = {"date": "2024-01-01", "merchant": "STORE", "amount": -50.0, "account": "Chase ••1234"}
        self.assertEqual(len(dedup([t1, t2])), 1)
...
class TestScanFolders(unittest.TestCase):
    ...
    def test_returns_raw_dicts_without_overrides(self):
        # scan_folders returns raw dicts; callers (get_all_transactions) apply overrides
        csv_content = "Date,Description,Amount\n2024-01-01,Test Store,-25.00\n"
        (self.expense_dir / "test.csv").write_text(csv_content)
        result = scan_folders([self.expense_dir])
        # No txn_id set — that's done by the caller
        self.assertNotIn("txn_id", result[0])
        # Category is whatever the parser set (e.g. "Uncategorized")
        self.assertEqual(result[0].get("category", "Uncategorized"), "Uncategorized")
