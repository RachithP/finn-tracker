...
class TestFindHeaderSkip(unittest.TestCase):

    def test_no_preamble_returns_zero(self):
        text = "Transaction Date,Post Date,Description,Category,Type,Amount,Memo\n03/18/2026,03/18/2026,STORE,,Sale,-50.00,"
        self.assertEqual(_find_header_skip(text), 0)

    def test_one_title_row_returns_one(self):
        text = "Chase_1234_all_transactions\nTransaction Date,Post Date,Description,Category,Type,Amount,Memo\n03/18/2026,..."
        self.assertEqual(_find_header_skip(text), 1)

    def test_capital_one_title_row(self):
        text = "savor_04_08_2025_to_04_08_2026\nTransaction Date,Posted Date,Card No.,Description,Category,Debit,Credit\n2026-03-22,..."
        self.assertEqual(_find_header_skip(text), 1)

    def test_blank_lines_before_title_still_finds_header(self):
        text = "\n\nSome Title\nTransaction Date,Post Date,Description,Category,Type,Amount,Memo\ndata"
        self.assertEqual(_find_header_skip(text), 3)

    def test_generic_date_synonym_matches(self):
        text = "preamble\ndate,merchant,amount\n2024-01-01,STORE,-50.00"
        self.assertEqual(_find_header_skip(text), 1)

    def test_raises_after_max_scan(self):
        lines = "\n".join([f"junk row {i}" for i in range(6)])
        with self.assertRaises(ValueError):
            _find_header_skip(lines)

    def test_raises_exactly_at_limit(self):
        # 5 non-empty non-header rows → should raise
        lines = "\n".join([f"junk {i}" for i in range(5)])
        with self.assertRaises(ValueError):
            _find_header_skip(lines)

    def test_header_on_fifth_row_is_accepted(self):
        # 4 preamble rows then the header on row 4 (0-indexed) — within limit
        rows = [f"preamble {i}" for i in range(4)]
        rows.append("Transaction Date,Post Date,Description,Category,Type,Amount,Memo")
        self.assertEqual(_find_header_skip("\n".join(rows)), 4)


class TestCSVParserWithTitleRow(unittest.TestCase):
    """parse_csv should handle CSVs that have a bank-exported title row before the header."""
    ...
    def test_chase_credit_with_title_row(self):
        csv_content = (
            "Chase_1234_all_transactions\n"
            "Transaction Date,Post Date,Description,Category,Type,Amount,Memo\n"
            "03/18/2026,03/18/2026,AUTOMATIC PAYMENT - THANK,,Payment,100.00,\n"
            "03/02/2026,03/03/2026,COFFEE SHOP,,Sale,-5.00,\n"
        )
        path = self._write(self.tmp, "chase.csv", csv_content)
        r = parse_csv(path)
        self.assertEqual(r.parser_used, "csv_chase_credit")
        self.assertEqual(len(r.transactions), 2)
        payment = next(t for t in r.transactions if t.amount > 0)
        self.assertAlmostEqual(payment.amount, 100.00)

    def test_capital_one_with_title_row(self):
        csv_content = (
            "savor_04_08_2025_to_04_08_2026\n"
            "Transaction Date,Posted Date,Card No.,Description,Category,Debit,Credit\n"
            "2026-03-22,2026-03-23,5678,CAPITAL ONE AUTOPAY PYMT,Payment/Credit,,284.95\n"
            "2026-03-21,2026-03-22,5678,COFFEE SHOP,Dining,9.70,\n"
        )
        path = self._write(self.tmp, "cap1.csv", csv_content)
        r = parse_csv(path)
        self.assertEqual(r.parser_used, "csv_capital_one")
        self.assertEqual(len(r.transactions), 2)
        payment = next(t for t in r.transactions if t.amount > 0)
        self.assertAlmostEqual(payment.amount, 284.95)

    def test_too_many_preamble_rows_records_error(self):
        csv_content = "\n".join([f"junk row {i}" for i in range(6)]) + "\n"
        path = self._write(self.tmp, "bad.csv", csv_content)
        r = parse_csv(path)
        self.assertTrue(any("No column headers" in e for e in r.errors))
...
class TestFlaskEndpoints(unittest.TestCase):
    ...
    def test_categories_update_valid(self):
        txn_id = flask_app.make_txn_id(SAMPLE_TXN[0])
...
class TestDeduplication(unittest.TestCase):
    ...
    def test_dedup_empty_list(self):
        self.assertEqual(flask_app.dedup([]), [])
    ...
    def test_dedup_charge_and_credit_not_deduplicated(self):
        ...
        result = flask_app.dedup([charge, credit])
        self.assertEqual(len(result), 2)
    ...
    def test_dedup_multiple_copies_collapse_to_one(self):
        ...
        result = flask_app.dedup([t, t, t, t])
        self.assertEqual(len(result), 1)
...
# ══════════════════════════════════════════════════════════════════════════════
# NEW: T1 — tests for eng-review fixes
# ══════════════════════════════════════════════════════════════════════════════

class TestInitAppIdempotent(unittest.TestCase):
    """init_app() can be called multiple times without error."""

    def test_init_app_idempotent(self):
        flask_app.init_app()
        flask_app.init_app()  # second call must not raise


class TestScanFoldersMtimeCache(unittest.TestCase):
    """db.scan_folders caches results by file mtime."""

    def test_cache_hit_skips_reparse(self):
        from finn_tracker.utils.db import scan_folders, _scan_cache
        from unittest.mock import patch

        with tempfile.TemporaryDirectory() as tmpdir:
            csv_path = Path(tmpdir) / "test.csv"
            csv_path.write_text(CHASE_BANK_CSV)

            folders = [Path(tmpdir)]
            call_count = []

            orig_ingest = __import__('finn_tracker.ingest', fromlist=['ingest_file']).ingest_file
            def counting_ingest(fp, **kw):
                call_count.append(fp)
                return orig_ingest(fp, **kw)

            with patch('finn_tracker.utils.db.scan_folders.__module__'):
                pass  # just ensuring import works

            import finn_tracker.utils.db as db_module
            with patch.object(db_module, '_scan_cache', {}):
                with patch('finn_tracker.ingest.ingest_file', side_effect=counting_ingest) as mock_ingest:
                    db_module.scan_folders(folders)
                    first_count = mock_ingest.call_count
                    db_module.scan_folders(folders)
                    second_count = mock_ingest.call_count

            self.assertEqual(first_count, 1)
            self.assertEqual(second_count, 1)  # no second parse — cache hit
    ...
    def test_cache_invalidated_on_mtime_change(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(db_module, '_scan_cache', {}):
                with patch('finn_tracker.ingest.ingest_file', wraps=__import__('finn_tracker.ingest', fromlist=['ingest_file']).ingest_file) as mock_ingest:
                    db_module.scan_folders(folders)
                    first_count = mock_ingest.call_count
            ...
            self.assertEqual(first_count, 1)
            self.assertEqual(second_count, 2)  # mtime changed → cache miss → re-parse


class TestImportFolderErrorHandling(unittest.TestCase):
    """import_folder catches per-file exceptions and reports them in errors."""

    def setUp(self):
        self.client = flask_app.app.test_client()

    def test_parse_exception_does_not_crash_endpoint(self):
        from unittest.mock import patch
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('finn_tracker.app.ingest_file', side_effect=Exception("boom")):
                resp = self.client.post('/import/folder', json={"folder": tmpdir})

        self.assertNotEqual(resp.status_code, 500)
        data = json.loads(resp.data)
        self.assertGreater(len(data.get("errors", [])), 0)
        self.assertIn("boom", data["errors"][0])


class TestBatchUpdateCategoriesMultiple(unittest.TestCase):
    """batch_update_categories persists all valid updates in one request."""
    ...
    def test_multiple_updates_all_applied(self):
        updates = [
            {"txn_id": f"testbatch{i:04d}", "category": "Food & Dining"}
            for i in range(5)
        ]
        resp = self.client.post('/categories/batch-update', json={"updates": updates})
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.data)
        self.assertEqual(data["updated"], 5)
        self.assertTrue(data["ok"])
        for i in range(5):
            self.assertEqual(flask_app._session["categories"].get(f"testbatch{i:04d}"), "Food & Dining")
